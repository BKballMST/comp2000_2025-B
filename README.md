...
This is every functionality in my assignment code explained. When you run the code, you might relise that there are only two pawns instead of 8 pawns. Becuase as mentioned above, this doesn't include every chess rules and gameplays of it, just recreating the chess board and each chess piece movement. Therefore no capturing pieces. And because of the pawn can't move backward, if the pawns stuck in the middle of the board, then you can't really move other piece across the board. I want to show you the full range movement of other pieces too rather than just the pawns.